...
enumfile = "veris-enum.json"
# file with the files expected (e.g. "action.hacking.variety", "actor.external.country" and "asset.assets.variety")
verisfields = "VERIS-fields.txt"
# the full JSON schema (from github)
verisschema = "veris-pub-v1.json"
...
for filename in glob.glob(fileglob):
    ...
    v = Draft3Validator(jdata, jschema)
    ...
    errList = dict()
    if jdata.has_key('security_compromise'):
        errList['security_compromise'] = compareFromTo(jdata['security_compromise'], edata['security_compromise'])
    ...
    if jdata.has_key('plus'):
        for method in ['attack_difficulty_legacy', 'attack_difficulty_initial', 'attack_difficulty_subsequent']:
            if jdata['plus'].has_key(method):
                errList['plus.' + method] = compareFromTo(jdata['plus'][method], edata['plus']['attack_difficulty'])
        for method in ['analysis_status', 'targeted', 'asset_os', 'unknown_unknowns', 'antiforensic_measure', 'unfollowed_policies', 'public_disclosure', 'control_inadequacies_legacy', 'security_maturity']:
            if jdata['plus'].has_key(method):
                errList['plus.' + method] = compareFromTo(jdata['plus'][method], edata['plus'][method])
        if jdata['plus'].has_key('pci'):
            for method in ['compliance_status', 'mercant_level']:
                if jdata['plus']['pci'].has_key(method):
                    errList['plus.pci.' + method] = compareFromTo(jdata['plus']['pci'][method], edata['plus']['pci'][method])
            for index in range(1,13):
                method= "req_" + str(index)
                if jdata['plus']['pci'].has_key(method):
                    errList['plus.pci.' + method] = compareFromTo(jdata['plus']['pci'][method], edata['plus']['pci']['requirement'])
    ...
    if jdata['security_compromise']=="Confirmed" and not len(jdata['attribute']):
        valError.append("No attributes listed, but security compromise is confirmed?")
