    ...
    for root, dirnames, filenames in tqdm(os.walk(cfg['input'])):
      ...
      if filenames:
        ...
        for fname in filenames:
            ...
            incident['sequence'][0]['action'] = incident.pop('action')
            incident['sequence'][0]['actor'] = incident.pop('actor')
            incident['sequence'][0]['asset'] = incident.pop('asset')
            incident['sequence'][0]['attribute'] = incident.pop('attribute')
            incident['sequence'][0]['victim'] = incident.pop('victim')
            incident['sequence'][0]['discovery_method'] = incident.pop('discovery_method')
            if 'confidence' in incident:
                incident['sequence'][0]['confidence'] = incident.pop('confidence')
            if 'control_failure' in incident.keys():
                incident['sequence'][0]['control_failure'] = incident.pop('control_failure')
            if 'discovery_notes' in incident.keys():
                incident['sequence'][0]['discovery_notes'] = incident.pop('discovery_notes')
            if 'compromise' in incident['timeline']:
                incident['sequence'][0]['timeline']['compromise'] = incident['timeline'].pop('compromise')
            if 'exfiltration' in incident['timeline']:
                incident['sequence'][0]['timeline']['exfiltration'] = incident['timeline'].pop('exfiltration')
            if 'discovery' in incident['timeline']:
                incident['sequence'][0]['timeline']['discovery'] = incident['timeline'].pop('discovery')
            if 'containment' in incident['timeline']:
                incident['sequence'][0]['timeline']['containment'] = incident['timeline'].pop('containment')
            if (len(incident['sequence'][0]['timeline']) == 0):
                _ = incident['sequence'][0].pop('timeline')
